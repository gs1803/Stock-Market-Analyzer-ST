...
class USEconomy:
    ...
    def inflation_rate() -> None:
        ...
        if inflationOption == 'Inflation Rate':
            ...
            StockAnalyzer.arrow_change(inflationDf, 'inflation_rate')

            fig = go.Figure(data = go.Scatter(x = inflationDf.index, y = inflationDf['inflation_rate']))

            fig.update_layout(xaxis_title = 'Date',
                              title = 'Inflation Rate')
            st.markdown(f"### Latest Inflation Rate: {inflationDf['inflation_rate'].iloc[-1]:.3f}% \
                        {StockAnalyzer.arrow_change(inflationDf, 'inflation_rate')}",
            ...
            st.markdown(f"### Latest Core Inflation Rate: {coreInflationDf['core_inflation_rate'].iloc[-1]:.3f}% \
                        {StockAnalyzer.arrow_change(coreInflationDf, 'core_inflation_rate')}",
                        unsafe_allow_html = True)
            st.plotly_chart(fig, use_container_width = True)
    ...
    def unemployment_rate() -> None:
        ...
        st.markdown(f"### Latest Unemployment Rate: {unemploymentDf['unemployment_rate'].iloc[-1]:.3f}% \
                    {StockAnalyzer.arrow_change(unemploymentDf, 'unemployment_rate')}",
                    unsafe_allow_html = True)
        st.plotly_chart(fig, use_container_width = True)
    ...
    def interest_rates() -> None:
        ...
        if interestOption == 'Federal Funds Effective Rate':
            ...
            st.markdown(f"### Latest Federal Funds Effective Rate: {fferDf['ffer'].iloc[-1]:.2f}% \
                        {StockAnalyzer.arrow_change(fferDf, 'ffer')}",
                        unsafe_allow_html = True)
            st.plotly_chart(fig, use_container_width = True)
        ...
        if interestOption == 'Market Yield on U.S. Treasury Securities (10 Year)':
            ...
            st.markdown(f"### Latest Market Yield on U.S. Treasury Securities (10 Year) Rate: {sofrDf['myuts'].iloc[-1]:.2f}% \
                        {StockAnalyzer.arrow_change(sofrDf, 'myuts')}",
                        unsafe_allow_html = True)
            st.plotly_chart(fig, use_container_width = True)
        ...
        if interestOption == 'Secured Overnight Financing Rate':
            ...
            st.markdown(f"### Latest Secured Overnight Financing Rate: {sofrDf['sofr'].iloc[-1]:.2f}% \
                        {StockAnalyzer.arrow_change(sofrDf, 'sofr')}",
                        unsafe_allow_html = True)
            sofrCol1, sofrCol2, sofrCol3 = st.columns([3, 3, 3])
            sofrCol1.markdown(f"30-day Average: {sofr30Df['sofr30'].iloc[-1]:.2f}% {StockAnalyzer.arrow_change(sofr30Df, 'sofr30')}", 
                              unsafe_allow_html = True)
            sofrCol2.markdown(f"90-day Average: {sofr90Df['sofr90'].iloc[-1]:.2f}% {StockAnalyzer.arrow_change(sofr90Df, 'sofr90')}", 
                              unsafe_allow_html = True)
            sofrCol3.markdown(f"180-day Average: {sofr180Df['sofr180'].iloc[-1]:.2f}% {StockAnalyzer.arrow_change(sofr180Df, 'sofr180')}", 
                              unsafe_allow_html = True)
            st.plotly_chart(fig, use_container_width = True)
    ...
    def exchange_rates() -> None:
        ...
        if usdEuro:
            ...
            fig.update_layout(xaxis_title = 'Date',
                              title = 'USD to EURO')
            st.markdown(f"### Latest USD to EURO Exchange Rate: {usdEuroDf['exchange_rate'].iloc[-1]:.3f} EUR\
                        {StockAnalyzer.arrow_change(usdEuroDf, 'exchange_rate')}",
                        unsafe_allow_html = True)
            st.plotly_chart(fig, use_container_width = True)
        ...
        if usdCad:
            ...
            fig.update_layout(xaxis_title = 'Date',
                              title = 'USD to CAD')
            st.markdown(f"### Latest USD to CAD Exchange Rate: {usdCadDf['exchange_rate'].iloc[-1]:.3f} CAD \
                        {StockAnalyzer.arrow_change(usdCadDf, 'exchange_rate')}",
                        unsafe_allow_html = True)
            st.plotly_chart(fig, use_container_width = True)
        ...
        if usdInr:
            ...
            fig.update_layout(xaxis_title = 'Date',
                              title = 'USD to INR')
            st.markdown(f"### Latest USD to INR Exchange Rate: {usdInrDf['exchange_rate'].iloc[-1]:.3f} INR \
                        {StockAnalyzer.arrow_change(usdInrDf, 'exchange_rate')}",
                        unsafe_allow_html = True)
            st.plotly_chart(fig, use_container_width = True)
        ...
        if usdYen:
            ...
            fig.update_layout(xaxis_title = 'Date',
                              title = 'USD to YEN')
            st.markdown(f"### Latest USD to YEN Exchange Rate: {usdYenDf['exchange_rate'].iloc[-1]:.3f} YEN \
                        {StockAnalyzer.arrow_change(usdYenDf, 'exchange_rate')}",
                        unsafe_allow_html = True)
            st.plotly_chart(fig, use_container_width = True)
        ...
        if usdRmb:
            ...
            fig.update_layout(xaxis_title = 'Date',
                              title = 'USD to RMB')
            st.markdown(f"### Latest USD to RMB Exchange Rate: {usdRmbDf['exchange_rate'].iloc[-1]:.3f} RMB \
                        {StockAnalyzer.arrow_change(usdRmbDf, 'exchange_rate')}",
                        unsafe_allow_html = True)
            st.plotly_chart(fig, use_container_width = True)
    ...
    def mortgage_rates() -> None:
        ...
        st.markdown(f"### Latest 15 Year Fixed Rate: {mortgage15Df['mort15'].iloc[-1]:.2f}% \
                    {StockAnalyzer.arrow_change(mortgage15Df, 'mort15')}", 
                    unsafe_allow_html = True)
        st.markdown(f"### Latest 30 Year Fixed Rate: {mortgage30Df['mort30'].iloc[-1]:.2f}% \
                    {StockAnalyzer.arrow_change(mortgage30Df, 'mort30')}",
                    unsafe_allow_html = True)
        st.plotly_chart(fig, use_container_width = True)
