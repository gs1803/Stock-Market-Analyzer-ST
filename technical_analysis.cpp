...
class TechnicalAnalysis {
    public:
        vector<double> rsi_calculation(const vector<double>& data, int lookback) {
            ...
            return vector<double>(rsi.begin() + 3, rsi.end());
        }
...
int main() {
    vector<double> data = {52.34, 49.87, 55.12, 58.76, 47.92, 50.45, 53.89, 48.67, 51.23, 54.78, 
                           56.91, 46.78, 49.32, 57.44, 52.67, 55.89, 47.56, 50.98, 58.23, 53.76};
    int lookback = 5;
    TechnicalAnalysis ta;
    vector<double> rsi = ta.rsi_calculation(data, lookback);

    auto [rsi_buy_prices, rsi_sell_prices] = ta.implement_rsi(data, rsi);

    cout << "RSI Buy Prices:\n";
    for (size_t i = 0; i < rsi_buy_prices.size(); ++i) {
        if (!isnan(rsi_buy_prices[i])) {
            cout << "Day " << i + 1 << ": " << rsi_buy_prices[i] << endl;
        }
    }

    cout << "RSI Sell Prices:\n";
    for (size_t i = 0; i < rsi_sell_prices.size(); ++i) {
        if (!isnan(rsi_sell_prices[i])) {
            cout << "Day " << i + 1 << ": " << rsi_sell_prices[i] << endl;
        }
    }
